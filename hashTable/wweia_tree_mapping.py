...
for index, row in merged.iterrows():
    ...
    # Get the wweia_food_category_description
    category = row['wweia_food_category_description']

    # Check if this category has already been added to the parent node
    if category not in added_categories[parent]:
        # If not, add a new node for this category under the parent
        Node(category, parent=parent)

        # And add the category to the set for this parent node
        added_categories[parent].add(category)
