...
def index():
    ...
    return render_template('LPmodel/index.html', data=categories_with_separator)


@app.route("/recommendations", methods=["GET", "POST"])
def recommendations():
    if request.method == "GET":
        return render_template('LPmodel/recommendations.html', test="get", data={})
        ...
        for key in constraints_mapping:  #checks if constraint mapping is checked
            constraint_name = constraints_mapping[key]
            
            # If constraint is not checked, set default values of 0. Else, set proper max value of constraint
            if request.form.get(key, False):  
                output["Main_List"][constraint_name] = {"max": max_values[constraint_name], "min": 0}  # assigning max values from mapping above
            else:
                pass
                # output["Main_List"][constraint_name] = {"max": 0, "min": 0}
        ...
        # TODO: Why doesn't the main_database file contain the cleanup?
        ...
        return render_template('LPmodel/recommendations.html', data=data_recommendation, html_output=output_as_html)
